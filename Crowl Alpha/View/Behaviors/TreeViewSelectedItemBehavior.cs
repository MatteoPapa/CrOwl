    ...
    public class TreeViewSelectedItemBehavior : Behavior<TreeView>
    {
        ...
        private void UpdateTreeViewItem(TreeViewItem item, bool recurse)
        {
            ...
            if (SelectedItem == model)
            {
                ...
                // Bring the item into view using Dispatcher to ensure UI is ready
        ...
        private void UpdateAllTreeViewItems()
        {
            ...
            {
                var tvi = treeView.ItemContainerGenerator.ContainerFromItem(item) as ItemsControl;
                if (tvi != null)
                    CollapseAllItems(tvi);
            }

            // Now proceed to update (expand/select) items
            foreach (var item in treeView.Items)
            {
        ...
        private void CollapseAllItems(ItemsControl parentItem)
        {
            if (parentItem == null)
                return;
            ...
            {
                var tvi = parentItem.ItemContainerGenerator.ContainerFromItem(item) as TreeViewItem;
                if (tvi != null)
                {
                    tvi.IsExpanded = false; // Collapse the node
                    CollapseAllItems(tvi);   // Recursively collapse its children
                }
            }
        }
